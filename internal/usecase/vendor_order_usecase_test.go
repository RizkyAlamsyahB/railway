...
func setupVendorOrderUseCase(t *testing.T) (
	...
	*mocks.MockRajaOngkirProvider,
	...
	uc := NewVendorOrderUseCase(vendorOrderRepo, orderRepo, shipmentRepo, paymentRepo, userRepo, rajaOngkir)
	return vendorOrderRepo, orderRepo, shipmentRepo, paymentRepo, userRepo, rajaOngkir, uc
}

// --- AcceptOrder tests ---

func TestVendorOrderUseCase_AcceptOrder_Success(t *testing.T) {
	vendorOrderRepo, orderRepo, _, _, _, _, uc := setupVendorOrderUseCase(t)
...
func TestVendorOrderUseCase_AcceptOrder_NotPaid(t *testing.T) {
	vendorOrderRepo, _, _, _, _, _, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
	orderID := uuid.New()
	userID := uuid.New()
...
func TestVendorOrderUseCase_ShipOrder_Success(t *testing.T) {
	vendorOrderRepo, orderRepo, shipmentRepo, _, _, _, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
	orderID := uuid.New()
	userID := uuid.New()
...
func TestVendorOrderUseCase_TrackWaybill_Success(t *testing.T) {
	vendorOrderRepo, _, shipmentRepo, _, _, rajaOngkir, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
	orderID := uuid.New()
	now := time.Now()
...
func TestVendorOrderUseCase_TrackWaybill_NoShipment(t *testing.T) {
	vendorOrderRepo, _, shipmentRepo, _, _, _, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
	orderID := uuid.New()
...
func TestVendorOrderUseCase_ListOrders_Success(t *testing.T) {
	vendorOrderRepo, orderRepo, _, _, userRepo, _, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
	userID := uuid.New()
	orderID := uuid.New()
...
func TestVendorOrderUseCase_ListOrders_InvalidStatus(t *testing.T) {
	_, _, _, _, _, _, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
...
func TestVendorOrderUseCase_GetOrderDetail_Success(t *testing.T) {
	vendorOrderRepo, orderRepo, shipmentRepo, paymentRepo, userRepo, _, uc := setupVendorOrderUseCase(t)
	ctx := context.Background()
	vendorID := uuid.New()
	orderID := uuid.New()
	userID := uuid.New()
	...
	order := &domain.Order{
		...
		GrandTotal:              100000,
		PlacedAt:                time.Now(),
		ShippingAddressSnapshot: map[string]interface{}{"city": "Jakarta"},
	}
	...
	userRepo.EXPECT().FindByID(ctx, userID).Return(&domain.User{ID: userID, FullName: "Jane Doe"}, nil)
	...
	if res.CustomerName != "Jane Doe" {
		t.Fatalf("expected Jane Doe, got %s", res.CustomerName)
	}
	if res.Shipment == nil {
		t.Fatalf("expected shipment info, got nil")
	}
	if res.Shipment.ETD != "2-3 day" {
		t.Fatalf("expected ETD '2-3 day', got %s", res.Shipment.ETD)
	}
	if res.Shipment.EstimatedArrival == nil {
		t.Fatalf("expected estimated arrival to be set")
	}
